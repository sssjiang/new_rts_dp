...
| 描述                                                         | 输入        | 关联输入            | 输出   |
| ------------------------------------------------------------ | ----------- | ------------------- | ------ |
| 提取这个在xpath路径下有多个元素的时候，可以直接将每个元素分开返回 | xpath表达式 | type:array          | list   |
| 提取这个在xpath路径下的字符串                                | xpath表达式 | type:string(默认值) | String |
...
| 描述                       | 输入        | 关联输入        | 输出                |
| -------------------------- | ----------- | --------------- | ------------------- |
| 定位之后的结果进行正则提取 | xpath表达式 | return:[n]默认0 | 返回每个匹配的第n个 |
...
| ------------------------ | ----------- | --------------------------- | --------------------------------------- |
| 保留定位处的原始网页代码 | xpath表达式 | 无                          | 返回xpath对应节点的html源码拼接后的字串 |
| 提取链接                 | xpath表达式 | 有                          | 返回链接                                |
